 ...
 *
 * Security model:
 *   - Token: 32 random bytes encoded as base64url = 43 chars, 256 bits of entropy.
 *     Generated by `generateInvitationToken()` in invitations/route.ts.
 *     אנטרופיה גבוהה הופכת ניחוש לבלתי מעשי; אין צורך בבדיקת uniqueness מול ה-DB.
 *   - OTP: 6 digits, hashed with bcrypt(10), never persisted in plaintext.
 *   - Max attempts: 5 → invitation auto-revoked.
 *   - Expiry: 48h from creation; resend renews to now+48h.
 */
...
export function isExpired(expiresAt: Date): boolean {
  // <=: ברגע ש-now מגיע ל-expiresAt בדיוק, ההזמנה פגה. סוגר חלון 1ms.
...
interface RestorePlan {
  newStatus: string;
  newTrialEndsAt: Date;
  newSubscriptionEndsAt: Date | null;
 ...
 * pure — מקבל את כל המצב כפרמטרים, מחזיר את הסטטוס + trialEndsAt + subscriptionEndsAt החדשים.
 *
 * החלטות:
 *   1. wasNeverActive (subscriptionStatusBeforeClinic=null) → TRIALING + 30 ימים חדשים.
 *      רציונל: המשתמש הצטרף ישירות דרך הזמנה ולא הספיק להחליט על מנוי.
 *   2. trialEndsAt או subscriptionEndsAt פגים → סטטוס קודם + grace 7 ימים על שניהם.
 *      רציונל: לא רוצים לחזור למצב חסום מיד; מאפשרים זמן לחדש תשלום.
 *   3. אחרת → סטטוס קודם + trialEndsAt + subscriptionEndsAt המקוריים.
 *
 * subscriptionEndsAt: לפני הקליניקה היה תקף לתאריך מסוים. בזמן ב-PAUSED הוא "קופא"
 * אבל התאריך הקלנדרי ממשיך — אז ייתכן שאחרי שחזור הוא כבר עבר. אם כן → grace.
 */
export function computeBillingRestore(params: {
  ...
  const graceMs = RESTORE_GRACE_DAYS * 24 * 60 * 60 * 1000;
  ...
  if (wasNeverActive) {
    return {
      ...
      newSubscriptionEndsAt: null, // אין מנוי ששולם בעבר
  ...
  const subEndsExpired =
    params.subscriptionEndsAt != null &&
    params.subscriptionEndsAt.getTime() < now.getTime();

  if (trialExpired || subEndsExpired) {
    const graceEnd = new Date(now.getTime() + graceMs);
    return {
      newStatus: params.subscriptionStatusBeforeClinic ?? "TRIALING",
      newTrialEndsAt: graceEnd,
      // אם subscriptionEndsAt פג — מאריכים גם אותו ל-grace.
      // אם הוא תקף עדיין (אבל ה-trial פג) — נשמר על המקור.
      newSubscriptionEndsAt: subEndsExpired
        ? graceEnd
        : params.subscriptionEndsAt ?? null,
  ...
  // trialEndsAt could be null (e.g., ACTIVE without trial); fallback to grace.
  return {
    newStatus: params.subscriptionStatusBeforeClinic ?? "TRIALING",
    newTrialEndsAt: params.trialEndsAt ?? new Date(now.getTime() + graceMs),
    newSubscriptionEndsAt: params.subscriptionEndsAt ?? null,
    grantedFreshTrial: false,
    appliedGrace: false,
  };
}
