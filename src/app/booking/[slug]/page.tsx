...
export default function BookingPage() {
  ...
  function renderCalendar() {
    ...
    maxDate.setDate(maxDate.getDate() + therapistInfo.maxAdvanceDays);
      ...
      <div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {cells.map((date, idx) => {
            ...
            const isEnabled = therapistInfo.enabledDays.includes(dayNum);
      ...
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <div className="text-center space-y-2">
          {therapistInfo.therapistImage && (
            <img
              src={therapistInfo.therapistImage}
              alt={therapistInfo.therapistName}
              className="w-20 h-20 rounded-full mx-auto object-cover"
            />
          )}
          <h1 className="text-2xl font-bold">{therapistInfo.therapistName}</h1>
          {therapistInfo.welcomeMessage && (
            <p className="text-muted-foreground">{therapistInfo.welcomeMessage}</p>
          )}
          <div className="flex items-center justify-center gap-1 text-sm text-muted-foreground">
            <Clock className="h-4 w-4" />
            <span>{therapistInfo.sessionDuration} דקות</span>
          </div>
        </div>
            ...
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors
                ${step === s ? "bg-primary text-primary-foreground" :
                  (["date", "time", "details"].indexOf(step) > i || step === "success")
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                ...
                בחירת שעה - {getDayLabel(selectedDate.getDay())} {selectedDate.toLocaleDateString("he-IL")}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {slotsLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <Button variant="link" onClick={() => setStep("date")} className="mt-2">
                    בחר/י יום אחר
                  </Button>
                </div>
              ) : (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="bg-muted/50 rounded-lg p-3 text-sm flex items-center gap-3">
                ...
                <span>{getDayLabel(selectedDate.getDay())} {selectedDate.toLocaleDateString("he-IL")}</span>
                <Clock className="h-4 w-4 text-primary shrink-0 mr-2" />
              ...
              <div className="space-y-2">
                ...
                <Input id="name" value={clientName} onChange={(e) => setClientName(e.target.value)} placeholder="השם שלך" required />
              </div>
              <div className="space-y-2">
                <Label htmlFor="phone" className="flex items-center gap-1">
                  <Phone className="h-3.5 w-3.5" /> טלפון
                </Label>
                <Input id="phone" type="tel" value={clientPhone} onChange={(e) => setClientPhone(e.target.value)} placeholder="050-1234567" dir="ltr" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="email" className="flex items-center gap-1">
                  <Mail className="h-3.5 w-3.5" /> אימייל
                </Label>
                <Input id="email" type="email" value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} placeholder="your@email.com" dir="ltr" />
              </div>
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="האם יש משהו שתרצה/י לשתף מראש?"
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button onClick={handleSubmit} disabled={!clientName || submitting} className="flex-1">
                  {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : "קביעת תור"}
                </Button>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-8 text-center space-y-4">
              ...
              <div className="bg-muted/50 rounded-lg p-4 space-y-2 text-sm">
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span>{therapistInfo.therapistName}</span>
                </div>
              </div>
            </CardContent>
          </Card>
