...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!staffBypass) {
      ...
      limit = tierLimits.combinedQuestionnaireLimit;

      if (limit === -1) {
        return NextResponse.json(
          {
            message: "ניתוח שאלונים משולב אינו זמין בתוכנית הנוכחית. שדרג את התוכנית שלך.",
            upgradeLink: "/dashboard/settings/billing",
          },
          { status: 403 }
        );
...
${getUniversalPromptsLight()}`;
    ...
    logger.error("שגיאה בניתוח שאלונים משולבים:", { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json(
