...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = await parseBody(request, aiSessionPrepSchema);
    if ("error" in parsed) return parsed.error;
    const { clientId, sessionDate } = parsed.data;
    ...
    // פסבדונים נשלח ל-AI במקום שם אמיתי (פרטיות). תאריך מעוצב בעברית במקום ISO גולמי.
    const clientPseudonym = getClientPseudonym(client.id);
    const sessionDateHe = formatSessionDateHe(sessionDate);
    ...
    // החזרת השם האמיתי לתצוגה (הפסבדונים נשלח רק ל-AI), כדי שלא יופיע קוד אנגלי לא מובן.
    const realClientName = (client.name || "").trim() || "המטופל";
    let content = sanitizeAiText(result.response.text());
    if (clientPseudonym && clientPseudonym.length >= 3) {
      content = content.split(clientPseudonym).join(realClientName);
    }
...
/**
 * עיצוב תאריך הפגישה לעברית ברורה (במקום ISO גולמי), בשעון ישראל.
 */
function formatSessionDateHe(raw?: string): string {
  const d = raw ? new Date(raw) : new Date();
  if (isNaN(d.getTime())) {
    return raw || format(new Date(), 'dd/MM/yyyy', { locale: he });
  }
