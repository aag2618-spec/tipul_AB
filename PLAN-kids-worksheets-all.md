...
## ✅ סבב 2 (2026-06-24) — הרחבה ל-75 דפים
תשתית חדשה **קובץ-לכל-דף**: `src/lib/worksheets-pages/{slug}.mjs` (דף שלם: עטיפת קטגוריה + content מבוגרים + psychoed/deepening/tracking + kids) → `scripts/gen-extra-pages.mjs` מרכז ל-`worksheets-content-extra3.mjs` → ממוזג ב-content.mjs. מאפשר סוכנים מקבילים בלי התנגשות.
- **11 דפי legacy הומרו** מ-baseCategories (HTML ידני) למערכת המאוחדת — קיבלו הדרכה מלאה + גרסת ילדים. ב-page.tsx נוסף סינון דינמי (contentSlugs) שמסיר אותם מ-baseCategories → אין כפילות.
- **25 דפים חדשים** נוצרו (נושאים שחסרו: עץ דאגות, חלון סבילות, גבולות, SMART, דחיינות, חוסן, אשמה/בושה, אבל, קבלה ועוד).
- סה"כ **75 דפים** בקטלוג (39 + 36), כולם עם גרסת ילדים. חיות חדשות: צב/עטלף/עופר/קורא/צפרדע/דישון/לויתן/יחמור/אתון/אנפה/דיה + קיפוד/תאו/זמר/קאת/קוף/אקו/חמור/רכש/חולד/חגב/שממית/יען/כבש/פרא/שלך/לביא/רחם/עיר/פרד/בקר/לטאה/תנין/פר/חרגול/ינשוף.
- שיטה: 2 Workflows + batches (כשלי 529/EOF הושלמו ב-retry). build+QA: 0 "מותר", כולם תקינים.
- ⚠️ PDF בייצור (150); **טרם push**; טרם נבדק חזותית.
