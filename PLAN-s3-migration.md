...
> **עדכון סטטוס (2026-06-02):** שלב 1 (S3Provider) ושלב 2 (העברת כל ה-callers
> של קבצי מטופלים למודול `storage`) — **בוצעו**. כל המסמכים, ההקלטות והצרופות
> כבר עוברים דרך `storage` ויעברו אוטומטית ל-S3 ברגע ש-`S3_BUCKET` יוגדר.
> נשאר רק: הקמת bucket + הגדרת env vars ב-Render — ראה
> **`HANDOFF-cloud-storage-setup.md`**. חריגים שעדיין על דיסק (לא PHI, טיפול
> נפרד): גיבוי PDF של Cardcom (`pdf-backup.ts`) ותבניות סטטיות של דפי עבודה.
