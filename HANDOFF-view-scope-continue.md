...
**סטטוס:** מתג + סימון מטפל עובדים ב: מטופלים, תשלומים (חובות+היסטוריה), קבלות, ממתינים-לסיכום, **פגישות**. סנכרון מתג↔מסנן המטפלים **ביומן — תוקן**.
...
3. **C (tooltip צף ביומן)** — **הבא בתור**. שיפור UX.
